...
RSpec.describe Canonical::Sync::AlchemicalProperties do
  ...
  context 'when preserve_existing_records is false' do
    subject(:perform) { described_class.perform(false) }

    let(:syncer) { described_class.new(false) }
    ...
    it 'instantiates itself' do
      perform
      expect(described_class).to have_received(:new).with(false)
    end
  ...
  context 'when preserve_existing_records is true' do
    subject(:perform) { described_class.perform(true) }
