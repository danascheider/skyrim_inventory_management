  ...
  describe '::perform' do
    ...
    context 'when preserve_existing_records is false' do
      ...
      context 'when there are no existing canonical weapons in the database' do
        ...
        it 'creates the associations to smithing materials where they exist', :aggregate_failures do
          perform
          expect(Canonical::Weapon.find_by(item_code: '00034182').smithing_materials.length).to eq 0
          expect(Canonical::Weapon.find_by(item_code: '0005BF06').smithing_materials.length).to eq 0
          expect(Canonical::Weapon.find_by(item_code: '000139B4').smithing_materials.length).to eq 3
          expect(Canonical::Weapon.find_by(item_code: 'XX00CFB6').smithing_materials.length).to eq 0
        end
      ...
      context 'when there are existing canonical weapon records in the database' do
        ...
        it "removes associations that don't exist in the JSON data" do
          item_in_json.canonical_weapons_tempering_materials.create!(
    ...
    context 'when preserve_existing_records is true' do
      ...
      before do
        create(:enchantment, name: 'Frost Damage')
        material_codes.each {|code| create(:canonical_material, item_code: code) }
        create(:canonical_weapons_tempering_material, weapon: item_in_json, material: create(:canonical_material))
        allow(described_class).to receive(:new).and_return(syncer)
      end
      ...
      it "doesn't destroy associations" do
        perform
        expect(item_in_json.reload.canonical_weapons_tempering_materials.length).to eq 2
      end
    end
