        ...
        context 'when the aggregate list has a matching item from a different list' do
          ...
          it 'returns the aggregate list item and the regular list item', :aggregate_failures do
            ...
            aggregate_list_item_actual, regular_list_item_actual = JSON.parse(response.body).map do |item_attrs|
              item_attrs.reject {|key, _value| %w[created_at updated_at].include?(key) }
            end

            aggregate_list_item_expected = aggregate_list.list_items.last.attributes.reject {|k, _v| %w[created_at updated_at].include?(k) }
            regular_list_item_expected   = shopping_list.list_items.last.attributes.reject {|k, _v| %w[created_at updated_at].include?(k) }
    ...
    context 'when authenticated' do
      ...
      context 'when all goes well' do
        ...
        it 'returns the aggregate list item and the regular list item', :aggregate_failures do
          ...
          aggregate_list_item_expected = aggregate_list.list_items.first.attributes.reject {|k, _v| %w[created_at updated_at].include?(k) }
          regular_list_item_expected   = shopping_list.list_items.first.attributes.reject {|k, _v| %w[created_at updated_at].include?(k) }
