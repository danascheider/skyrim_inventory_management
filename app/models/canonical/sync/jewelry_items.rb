...
module Canonical
  module Sync
    class JewelryItems
      ...
      def perform
        raise Canonical::Sync::PrerequisiteNotMetError.new('Prerequisite not met: sync canonical materials and enchantments before jewelry items') unless Enchantment.any? && Canonical::Material.any?
        ...
        ActiveRecord::Base.transaction do
          ...
          json_data.each do |object|
            ...
            if !preserve_existing_records
              ...
              model.canonical_jewelry_items_enchantments.where.not(enchantment_id: enchantment_ids).destroy_all
            end

            # create enchantments
            object[:enchantments].each do |enchantment|
              join_model          = model
                                      .canonical_jewelry_items_enchantments
            ...
            if !preserve_existing_records
              codes        = object[:materials].pluck(:item_code)
              material_ids = Canonical::Material.where(item_code: codes).ids
              model.canonical_jewelry_items_materials.where.not(material_id: material_ids).destroy_all
            end

            # create materials
            object[:materials].each do |material|
              join_model          = model
                                      .canonical_jewelry_items_materials
              ...
              Rails.logger.error "Validation error saving associations: #{e.message}"
              raise e
            end
          end
